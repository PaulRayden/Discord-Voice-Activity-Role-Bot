...
* **Configuración Centralizada:** Toda la configuración específica de la web se encuentra en el archivo `config.php`.
...
* **Para la interfaz web:**
    ...
    * **Configuración de la Aplicación de Discord en el Portal de Desarrolladores:** Necesitarás configurar una **URL de retorno** que coincida con la ruta a `callback.php` en tu servidor web.
...
1.  **Configura el Bot de Discord (archivo `bot.py`):**
...
2.  **Configura la Interfaz Web (archivo `config.php`):**
    ...
    * **Reemplaza los marcadores de posición** con la información de tu aplicación de Discord y tu servidor web:
        ...
        * `$clientSecret`: El Client Secret de tu aplicación de Discord.
        * `$redirectUri`: La URL completa a `callback.php` en tu servidor web (ej., `https://tu-dominio.com/callback.php`).
        * `$adminUserId`: El ID del usuario administrador del bot (debe coincidir con el configurado en `bot.py`).
        * `$logFilePath`: La ruta al archivo `discord_bot.log` generado por el bot (ajusta según la ubicación de tus archivos).
...
1.  Sube todos los archivos PHP (`index.php`, `login.php`, `callback.php`, `panel.php`, `logout.php`, `config.php`) y el archivo `style.css` a tu servidor web.
...
* **Rutas de Archivos:** Asegúrate de que la ruta a `discord_bot.log` en `config.php` sea correcta y que el servidor web tenga permisos de lectura para ese archivo.
